 ...
 * @brief Tests for matching engine and execution models
 */
...
#include "hqt/matching/matching_engine.hpp"
...
protected:
    Tick tick{1'000'000, 1, 1'100'000, 1'100'020, 1'000'000, 1'000'000, 20};  // Timestamp, symbol_id, bid, ask, bid_vol, ask_vol, spread
    SymbolInfo eurusd;
    ...
    void SetUp() override {
        ...
        eurusd.tick_size = 1.0;
        eurusd.tick_value = 1.0;
...
protected:
    ...
    std::unique_ptr<MatchingEngine> create_engine() {
...
TEST_F(SlippageModelTest, ZeroSlippage) {
    ZeroSlippage model;
    auto slippage = model.calculate(OrderSide::BUY, 1.0, tick, eurusd, rng);
...
TEST_F(SlippageModelTest, FixedSlippage) {
    ...
    auto slippage = model.calculate(OrderSide::BUY, 1.0, tick, eurusd, rng);
    EXPECT_EQ(slippage, 2 * 100000);  // 2 points * point_value
}

TEST_F(SlippageModelTest, RandomSlippageRange) {
    RandomSlippage model(1, 5);
    auto slippage = model.calculate(OrderSide::BUY, 1.0, tick, eurusd, rng);
    EXPECT_GE(slippage, 1 * 100000);
    EXPECT_LE(slippage, 5 * 100000);
}

TEST_F(SlippageModelTest, VolumeSlippage) {
    VolumeSlippage model(1, 0.5);  // 1 point base + 0.5 points per lot

    // 1 lot: 1 + 0.5*1 = 1.5 points
    auto slippage1 = model.calculate(OrderSide::BUY, 1.0, tick, eurusd, rng);
    EXPECT_EQ(slippage1, 2 * 100000);  // Rounded to 2 points

    // 10 lots: 1 + 0.5*10 = 6 points
    auto slippage10 = model.calculate(OrderSide::BUY, 10.0, tick, eurusd, rng);
    EXPECT_EQ(slippage10, 6 * 100000);
}
...
TEST_F(CommissionModelTest, ZeroCommission) {
    ...
    auto commission = model.calculate(OrderSide::BUY, 1.0, 1'100'000, eurusd);
...
TEST_F(CommissionModelTest, FixedPerLot) {
    ...
    // 1 lot
    auto commission1 = model.calculate(OrderSide::BUY, 1.0, 1'100'000, eurusd);
    EXPECT_EQ(commission1, 7'000'000);

    // 2 lots
    auto commission2 = model.calculate(OrderSide::BUY, 2.0, 1'100'000, eurusd);
...
TEST_F(CommissionModelTest, FixedPerTrade) {
    ...
    auto commission10 = model.calculate(OrderSide::BUY, 10.0, 1'100'000, eurusd);
...
TEST_F(CommissionModelTest, TieredCommission) {
    ...
    // 5 lots: $7/lot
    auto commission5 = model.calculate(OrderSide::BUY, 5.0, 1'100'000, eurusd);
    EXPECT_EQ(commission5, 35'000'000);

    // 20 lots: $5/lot
    auto commission20 = model.calculate(OrderSide::BUY, 20.0, 1'100'000, eurusd);
    EXPECT_EQ(commission20, 100'000'000);

    // 100 lots: $3/lot
    auto commission100 = model.calculate(OrderSide::BUY, 100.0, 1'100'000, eurusd);
...
TEST_F(SwapModelTest, ZeroSwap) {
    ZeroSwap model;
    auto swap = model.calculate(OrderSide::BUY, 1.0, 1'100'000, 1'110'000, eurusd, 1);
    EXPECT_EQ(swap, 0);
...
TEST_F(SwapModelTest, StandardSwapPoints) {
    ...
    auto swap_long = model.calculate(OrderSide::BUY, 1.0, 1'100'000, 1'110'000, eurusd, 1);
    EXPECT_LT(swap_long, 0);  // Negative = charge

    // Short position (earn swap)
    auto swap_short = model.calculate(OrderSide::SELL, 1.0, 1'100'000, 1'090'000, eurusd, 1);
...
// MONEY swap type not supported (would require extending SwapType enum in symbol_info.hpp)
// For direct currency swaps, use PercentageBasedSwap with appropriate rates
...
TEST_F(SwapModelTest, TripleSwapMultiplier) {
    ...
    int64_t wednesday = 1'000'000;  // Adjust timestamp to be Wednesday
    EXPECT_EQ(model.get_multiplier(wednesday), 1);  // Need actual Wednesday timestamp
}
...
TEST_F(SwapModelTest, IslamicSwap) {
    ...
    auto swap0 = model.calculate(OrderSide::BUY, 1.0, 1'100'000, 1'110'000, eurusd, 0);
    EXPECT_EQ(swap0, 0);

    auto swap1 = model.calculate(OrderSide::BUY, 1.0, 1'100'000, 1'110'000, eurusd, 1);
    EXPECT_EQ(swap1, 0);

    // After grace period: fee applies
    auto swap2 = model.calculate(OrderSide::BUY, 1.0, 1'100'000, 1'110'000, eurusd, 2);
    EXPECT_EQ(swap2, 5'000'000);  // 1 billable day

    auto swap5 = model.calculate(OrderSide::BUY, 1.0, 1'100'000, 1'110'000, eurusd, 5);
...
TEST_F(SpreadModelTest, HistoricalSpread) {
    ...
    int64_t expected_spread = std::max(actual_spread, 10 * 100000LL);
...
TEST_F(SpreadModelTest, TimeOfDaySpread) {
    ...
    // Test different times (would need actual UTC timestamps)
    auto spread = model.calculate(tick, eurusd, 1'000'000, rng);
    EXPECT_GT(spread, 0);
}
...
TEST_F(SpreadModelTest, RandomSpread) {
    ...
    auto spread = model.calculate(tick, eurusd, 1'000'000, rng);
    EXPECT_GE(spread, 5 * 100000);  // At least minimum
}
...
TEST_F(MatchingEngineTest, ConstructionRequiresAllModels) {
    // Missing models should throw
    EXPECT_THROW(
        MatchingEngine(nullptr, nullptr, nullptr, nullptr, 0),
...
TEST_F(MatchingEngineTest, MarketOrderExecution) {
    ...
    // Buy at ask
    auto result = engine->execute_market_order(OrderSide::BUY, 1.0, tick, eurusd);
    ...
    // Sell at bid
    result = engine->execute_market_order(OrderSide::SELL, 1.0, tick, eurusd);
    EXPECT_TRUE(result.executed);
    EXPECT_LT(result.fill_price, tick.bid);  // Bid - slippage
}

TEST_F(MatchingEngineTest, LimitOrderTrigger) {
    auto engine = create_engine();
    Tick tick{1'000'000, 1, 1'100'000, 1'100'020, 1'000'000, 1'000'000, 20};

    PendingOrder buy_limit{};
    buy_limit.type = OrderType::LIMIT;
...
TEST_F(MatchingEngineTest, LimitOrderDoesNotTrigger) {
    auto engine = create_engine();
    Tick tick{1'000'000, 1, 1'100'000, 1'100'020, 1'000'000, 1'000'000, 20};
...
TEST_F(MatchingEngineTest, PositionTakeProfitTrigger) {
    auto engine = create_engine();
...
TEST_F(MatchingEngineTest, GapScenarioFillsAtGapPrice) {
    auto engine = create_engine();
...
TEST_F(MatchingEngineTest, DeterministicExecution) {
    ...
    auto result1 = engine1->execute_market_order(OrderSide::BUY, 1.0, tick, eurusd);
    auto result2 = engine2->execute_market_order(OrderSide::BUY, 1.0, tick, eurusd);
...
TEST_F(MatchingEngineTest, SwapCalculation) {
